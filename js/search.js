      ...
      if (markerData) {
        ...
        const content = `
          <div class="infoWindow ${
            markerData.usableBed > 0 ? "greenOverlay" : "redOverlay"
          }">
            <div class="box">
              <h4 class="name">${hospital.name}</h4>
              <p class="tel">${hospital.tel}</p>
              <p class="bedCount ${
                markerData.usableBed > 0 ? "greenBed" : "redBed"
              }">${markerData.usableBed}/${markerData.totalBed}</p>
              <p class="updateTime">갱신시간 : ${markerData.updateTime.substr(
                8,
                2
              )}시 ${markerData.updateTime.substr(10, 2)}분</p>
            </div>
